...
function isAllowedWsOrigin(origin, request) {
  ...
  const allowedOrigins = readListEnv('WS_ALLOWED_ORIGINS').map(normalizeOrigin).filter(Boolean);
...
function attachRealtimeGame(server) {
  ...
  const messageLimiter = createRateLimiter({
    limit: readNumberEnv('WS_MESSAGE_LIMIT', 120, { min: 10, integer: true }),
    windowMs: readNumberEnv('WS_MESSAGE_WINDOW_MS', 30_000, { min: 1000, integer: true })
  });
  const wss = new WebSocket.Server({
    server,
    maxPayload,
    verifyClient(info) {
      return isAllowedWsOrigin(info.origin, info.req);
    }
  });
  ...
  const ROOM_EMPTY_TTL_MS = readNumberEnv('ROOM_EMPTY_TTL_MS', 5 * 60 * 1000, { min: 10_000, integer: true });
  const ROOM_IDLE_TTL_MS = readNumberEnv('ROOM_IDLE_TTL_MS', 60 * 60 * 1000, { min: 60_000, integer: true });
  const ROOM_SWEEP_INTERVAL_MS = readNumberEnv('ROOM_SWEEP_INTERVAL_MS', 30 * 1000, { min: 1000, integer: true });
  const OFFLINE_TAKEOVER_MS = readNumberEnv('OFFLINE_TAKEOVER_MS', 60 * 1000, { min: 5_000, integer: true });
  const OFFLINE_TAKEOVER_SWEEP_MS = readNumberEnv('OFFLINE_TAKEOVER_SWEEP_MS', 10 * 1000, { min: 1000, integer: true });
  const DISCONNECT_GRACE_MS = readNumberEnv('DISCONNECT_GRACE_MS', 5 * 1000, { min: 1000, integer: true });
  const WS_SERVER_PING_MS = readNumberEnv('WS_SERVER_PING_MS', 30 * 1000, { min: 5_000, integer: true });
  ...
  function createPlayer({ id, name, ws = null, isBot = false, botIndex = 0, avatar = '', room = null }) {
    ...
    return {
      ...
      reconnectToken: isBot ? null : makeReconnectToken()
    };
  }
  ...
  function shouldWritePlayLog(text) {
    const value = String(text || '');
    if (value.includes(' 出 ')) return true;
    if (value.includes('传牌完成') || value.includes('换牌完成')) return true;
    if (/^第\s*\d+\s*墩：/.test(value) && value.includes('收墩')) return true;
    return false;
  }

  function addLog(room, text) {
    if (!shouldWritePlayLog(text)) return;
  ...
  function addInteraction(room, senderIndex, payload = {}) {
    ...
    addLog(room, `${item.from} 对 ${item.to} 发送互动：${item.label}`);
    return item;
  }
  
  
  function addAIInteraction(room, senderIndex, targetIndex, payload = {}) {
    if (!room || !room.players?.[senderIndex]?.isBot || !room.players?.[targetIndex]) return null;
    const key = `${room.roundNo}:${room.trickNo}:${senderIndex}:${targetIndex}:${payload.kind || 'emoji'}:${payload.label || ''}`;
    if (room.lastAiInteractionKey === key) return null;
    room.lastAiInteractionKey = key;
    return addInteraction(room, senderIndex, { toIndex: targetIndex, ...payload });
  }
  ...
  function canStillShootMoon(room, candidateIndex) {
    if (!room?.players?.[candidateIndex]) return false;
    return room.players.every((player, index) => (
      index === candidateIndex || Number(player.round || 0) === 0
    ));
  }
  ...
  function lastTrickOpponentVoidSuits(room, playerIndex) {
    const last = room.lastTrick;
    if (!last || !last.leadSuit || !Array.isArray(last.cards) || !last.cards.length) return new Set();
    const voidSuits = new Set();
    for (const play of last.cards) {
      if (play.player === playerIndex) continue;
      if (play.card?.suit && play.card.suit !== last.leadSuit) voidSuits.add(last.leadSuit);
    }
    return voidSuits;
  }

  function avoidOpponentVoidLeadSuits(room, playerIndex, cards, shootMoon) {
    if (shootMoon || !Array.isArray(cards) || cards.length <= 1) return cards || [];
    const voidSuits = lastTrickOpponentVoidSuits(room, playerIndex);
    if (!voidSuits.size) return cards;
    const safer = cards.filter(card => !voidSuits.has(card.suit));
    return safer.length ? safer : cards;
  }
  ...
  function chooseLeadCard(room, playerIndex, legal, shootMoon) {
    ...
    if (candidates.length) {
      const leadCandidates = avoidOpponentVoidLeadSuits(room, playerIndex, candidates, shootMoon);
      const smallOther = smallSafeOtherSuitLead(room, leadCandidates);
      ...
      if (tempoHeart && leadCandidates.includes(tempoHeart) && candidates.every(card => card.rank > 5 || card.suit === 'H')) return tempoHeart;
  
      // 高手风格：优先从长门出低张，减少被迫收分；短门保留给后续垫分/避分。
      const suitGroups = Object.keys(SUITS)
        .map(suit => ({ suit, cards: leadCandidates.filter(card => card.suit === suit) }))
    ...
    const tempoHeart = lowHeartLeadForTempo(room, legal);
    const fallback = avoidOpponentVoidLeadSuits(room, playerIndex, low, shootMoon);
  ...
  function scheduleAutoForcedCard(room) {
    ...
    const isOpeningClub2 = room.trickNo === 0 && room.trick.length === 0 && cardId === 'C2';
    if (!isLastCard && !isOpeningClub2) return false;
    const timer = setTimeout(() => {
      ...
      const currentIsLastCard = (current.hand || []).length === 1;
      const currentIsOpeningClub2 = room.trickNo === 0 && room.trick.length === 0 && cardId === 'C2';
      if (currentLegal.length === 1 && currentLegal[0].id === cardId && (currentIsLastCard || currentIsOpeningClub2)) {
        playCard(room, playerIndex, cardId);
      }
    }, isOpeningClub2 ? 620 : 520);
  ...
  function isHumanSocketActive(player) {
    return Boolean(player && !player.isBot && player.ws && player.ws.readyState === WebSocket.OPEN);
  }

  function reconnectTokenFrom(message) {
    return String(message?.reconnectToken || '').trim();
  }

  function hasValidReconnectToken(message, player) {
    return Boolean(player?.reconnectToken && reconnectTokenFrom(message) === player.reconnectToken);
  }

  function sendReconnectRequired(ws) {
    sendError(ws, '重连凭证无效，请重新创建或加入房间。');
  }

  function findHumanByNormalizedName(room, nickname, predicate = () => true) {
    const normalized = normalizeNickname(nickname);
    if (!normalized) return -1;
    return room.players.findIndex(player => (
      !player.isBot &&
      normalizeNickname(player.name) === normalized &&
      predicate(player)
    ));
  }

  function restoreDisconnectedHumanByName(ws, room, playerIndex, clientId, msg) {
    const player = room.players[playerIndex];
    if (!player || player.isBot || player.leftRoom || isHumanSocketActive(player)) return false;
    ...
    player.disconnectGraceTimer = null;
    player.disconnectGraceStartedAt = null;
    if (room.hostId === previousId) room.hostId = clientId;

    attachSocketToPlayer(ws, room, playerIndex);
    touchRoom(room);
    addLog(room, `${player.name} 已通过同名身份恢复原座位。`);
    broadcast(room);
    return true;
  }
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === 'hello') {
      ...
      if (room) {
        const index = room.players.findIndex(player => player.id === ws.clientId && !player.isBot);
        if (index >= 0) {
          if (!hasValidReconnectToken(msg, room.players[index])) return sendReconnectRequired(ws);
    ...
    if (msg.type === 'createRoom') {
      ...
      if (existing && existing.room.phase !== 'gameEnd') {
        if (!hasValidReconnectToken(msg, existing.room.players[existing.playerIndex])) return sendReconnectRequired(ws);
        attachSocketToPlayer(ws, existing.room, existing.playerIndex);
        send(ws, { type: 'roomCreated', roomId: existing.room.id, reconnectToken: existing.room.players[existing.playerIndex].reconnectToken || '' });
      ...
      send(ws, { type: 'roomCreated', roomId: room.id, reconnectToken: host.reconnectToken || '' });
      broadcast(room);
      return;
    }
    ...
    if (msg.type === 'joinRoom') {
      ...
      if (existingIndex >= 0) {
        if (!hasValidReconnectToken(msg, room.players[existingIndex])) return sendReconnectRequired(ws);
      ...
      if (takeoverIndex >= 0) {
        replaceTakeoverBotWithHuman(ws, room, takeoverIndex, clientId, nickname, msg);
        return;
      }
      ...
      const onlineSameNameIndex = findHumanByNormalizedName(room, nickname, player => (
        player.id !== clientId &&
        !player.leftRoom &&
        isHumanSocketActive(player)
      ));
      if (onlineSameNameIndex >= 0) return sendError(ws, '该昵称已有在线玩家使用，请更换昵称或使用原设备重连');

      const disconnectedSameNameIndex = findHumanByNormalizedName(room, nickname, player => (
        player.id !== clientId &&
        !player.leftRoom &&
        !isHumanSocketActive(player)
      ));
      if (disconnectedSameNameIndex >= 0) {
        restoreDisconnectedHumanByName(ws, room, disconnectedSameNameIndex, clientId, msg);
        return;
      }

      const usedSameNameIndex = findHumanByNormalizedName(room, nickname, player => player.id !== clientId);
      if (usedSameNameIndex >= 0) return sendError(ws, '该昵称已被房间内玩家使用，请更换昵称');
  ...
  function socketRateKey(ws) {
    return `${ws.remoteAddress || 'unknown'}:${ws.clientId || 'anonymous'}`;
  }
  ...
  wss.on('connection', (ws, request) => {
    ws.isAlive = true;
    ws.remoteAddress = getRemoteAddress(request);
    ...
    ws.on('message', raw => {
      if (!messageLimiter.allow(socketRateKey(ws))) {
        sendError(ws, '消息过于频繁，请稍后重试');
        try { ws.close(1008, 'rate limited'); } catch (error) { /* ignore */ }
        return;
      }
  ...
  const autoTakeoverTimer = setInterval(sweepAutoTakeovers, OFFLINE_TAKEOVER_SWEEP_MS);
  const roomSweepTimer = setInterval(sweepExpiredRooms, ROOM_SWEEP_INTERVAL_MS);
  wsHeartbeatTimer.unref?.();
  autoTakeoverTimer.unref?.();
